...
def test_extreme_prices_never_crash_the_board():
    from app.sportsbook.odds import format_american, implied_probability
    # the poison case from production: a consensus that rounds to 1.0000
    assert format_american("1.0000") == "-100000"
    assert format_american("1.0001") == "-100000"
    assert implied_probability("1.0000") == 1
    # and the provider never emits one in the first place
    ev = {
        "id": "x", "home_team": "Giant", "away_team": "Minnow",
        "bookmakers": [
            _book("b1", [("Giant", -100000), ("Minnow", 60000)]),
            _book("b2", [("Giant", -80000), ("Minnow", 50000)]),
        ],
    }
    mkts = P._markets(ev)
    h2h = next(m for m in mkts if m.type == "h2h")
